...
if(isset($_POST["action"]))
{
 ...
 if($_POST["action"] == "mun"){
  $query = "SELECT add_ward.ward_id,add_ward.ward_number FROM add_ward,municipality,district,zone,provience,country WHERE add_ward.fk_mun_id = municipality.mun_id AND municipality.fk_district_id = district.district_id AND district.fk_zone_id = zone.zone_id AND zone.fk_provience_id = provience.provience_id AND provience.fk_country_id = country.country_id AND municipality.mun_id= '".$_POST["query"]."'";
  $result = mysqli_query($con, $query);
